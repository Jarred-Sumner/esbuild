...
async function installBinaryFromPackage(
  ...
  if (!buffer) {
    const url = `https://github.com/Jarred-Sumner/esbuild/releases/download/pluginbuild/${name}-${version}.tgz`;
...
function installOnUnix(name: string): void {
  if (process.env.ESBUILD_BIN_PATH_FOR_TESTS) {
    ...
    installBinaryFromPackage(name, "bin/esbuild", binPath).catch((e) =>
      setImmediate(() => {
        throw e;
      })
    );
...
`
  ...
  if (process.env.ESBUILD_BIN_PATH_FOR_TESTS) {
    ...
    installBinaryFromPackage(name, "esbuild.exe", exePath).catch((e) =>
      setImmediate(() => {
        throw e;
      })
    );
